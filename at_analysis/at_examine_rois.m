...
for i=1:numel(d),
	...
	for j=1:2,
		...
		box off;
		[tpr,fpr,th] = roc([ones(1,numel(I)) zeros(1,numel(I_))],[[roiprop_{j}.ROIparameters.params3d(I).Volume] [roiprop_{j}.ROIparameters.params3d(I_).Volume]]);
		acc = (tpr+(1-fpr))/2; % if we had equal mixtures
		[maxacc,maxaccloc] = max(acc);
		disp(['ROI set ' int2str(j) ': Max accuracy is ' num2str(maxacc) ' at volume of ' num2str(th(maxaccloc)) ' (equal mixture assumption).']);
		hold on;
		plot(log10(th(maxaccloc))*[1 1],[0 0.1],'k--');
