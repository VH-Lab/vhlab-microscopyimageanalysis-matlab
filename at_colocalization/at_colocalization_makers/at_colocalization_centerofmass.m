...
if nargin==0,
	out{1} = {'distance_threshold','roi_set_2'};
	out{2} = {'Distance threshold (pixels) that determines when 2 ROIs will be considered colocalized.' , 'Name of second ROI set with which to compute overlap (leave blank to choose)'};
...
if ischar(parameters),
	switch lower(parameters),
		...
		case 'choose_inputdlg',
			...
			default_parameters.threshold = 5;
...
com_a = ROI_3d_centerofmass(rois{1}.CC, IM{1});
com_b = ROI_3d_centerofmass(rois{2}.CC, IM{2});

distances = L2_distance(com_a,com_b);

overlap_thresh = (distances <= parameters.threshold);
...
overlapped_objects = sum(overlap_thresh(:))/2; % divide by 2 because a(i,j) == a(j,i) but is only 1 overlap
...
h(end+1) = struct('parent',input_itemname,'operation','at_colocalization_centerofmass','parameters',parameters,...
	'description',['Found ' int2str(overlapped_objects) ' CLs with distance threshold = ' num2str(parameters.threshold) ' pixels of ROI ' input_itemname ' onto ROI ' parameters.roi_set_2 '.']);
