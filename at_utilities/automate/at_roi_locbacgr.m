...
%% SOUTH
if peak_loc(1,1) < size(img_stack,1) - (parameters.dist_cardinal + 1) %1000x1000, and we're drawing 50 pixels out
    for pix = 1:parameters.dist_cardinal,
    if peak_loc(1,1)+pix > max(pixel_locs(:,1))
        lines.southline(pix) = img_stack(peak_loc(1,1)+pix,peak_loc(1,2),peak_loc(1,3));
    end, end
else %1000x1000, if we're too close to get 50, we'll get what we can
    for pix = 1:size(img_stack,1)-peak_loc(1,1)-1
    if peak_loc(1,1)+pix > max(pixel_locs(:,1))
        lines.southline(pix) = img_stack(peak_loc(1,1)+pix,peak_loc(1,2),peak_loc(1,3));
    end, end
end
...
%% EAST
if peak_loc(1,2) < size(img_stack,2) - (parameters.dist_cardinal + 1) %1000x1000, and we're drawing 50 pixels out
    for pix = 1:parameters.dist_cardinal,
    if peak_loc(1,2)+pix > max(pixel_locs(:,2))
        lines.eastline(pix) = img_stack(peak_loc(1,1),peak_loc(1,2)+pix,peak_loc(1,3));
    end, end
else %1000x1000, if we're too close to get 50, we'll get what we can
    for pix = 1:size(img_stack,2)-peak_loc(1,2)-1
    if peak_loc(1,2)+pix > max(pixel_locs(:,2))
        lines.eastline(pix) = img_stack(peak_loc(1,1),peak_loc(1,2)+pix,peak_loc(1,3));
    end, end
end
