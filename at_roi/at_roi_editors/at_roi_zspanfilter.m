...
% 
%  OUT = AT_ROI_ZSPANFILTER(ATD, INPUT_ITEMNAME, OUTPUT_ITEMNAME, PARAMETERS)
%
...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = at_roi_zspanfilter;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = at_roi_zspanfilter(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
...
h(end+1) = struct('parent',input_itemname,'operation','at_roi_zspanfilter','parameters',parameters,...
