function out = doublethresholdmask(mdir, input_itemname, mask_itemname, output_itemname, parameters)
% DOUBLETHRESHOLDMASK - Threshold an image based on a mask and store results
%  
%  OUT = MIA.IMAGE.PROCESS.AT_IMAGE_DOUBLETHRESHOLD(MDIR, INPUT_ITEMNAME, MASK_ITEMNAME, OUTPUT_ITEMNAME, PARAMETERS)
%
...
%
%  MIA.IMAGE.PROCESS.DOUBLETHRESHOLD has several parameters:
	...
	switch lower(parameters),
		case 'choose',
			...
			if ~strcmp(buttonname,'Cancel'),
				out = mia.image.process.doublethreshold(mdir,input_itemname,mask_itemname, output_itemname,buttonname);
			else,
				out = [];
			end;
		case {'choose_inputdlg'},
			...
			if isempty(parameters),
				...
				out = mia.image.process.doublethreshold(mdir,input_itemname,mask_itemname,output_itemname,parameters);
			end;
...
im_mask_file = mdir.getimagefilename(mask_itemname);
