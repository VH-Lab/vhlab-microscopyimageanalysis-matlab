...
% 
%  OUT = MIA.COLOCALIZATION.MAKERS.AT_COLOCALIZATION_SHIFTXYZ(ATD, INPUT_ITEMNAME, OUTPUT_ITEMNAME, PARAMETERS)
%
...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = mia.colocalization.makers.at_colocalization_shiftxyz;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = mia.colocalization.makers.at_colocalization_shiftxyz(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
		case 'choose_inputdlg',
			out_p = mia.colocalization.makers.at_colocalization_shiftxyz;
...
h(end+1) = struct('parent',input_itemname,'operation','mia.colocalization.makers.at_colocalization_shiftxyz','parameters',parameters,...
