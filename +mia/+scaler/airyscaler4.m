function airyscaler = airyscaler4(fname,varargin)
...
% Deconvolve to stored PSF
    load('bead_psf.mat');
    [img_stack_deconvolved(:,:,k),psfout] = deconvblind(img_stack_normalized(:,:,k),psf);
%Time bar
...
plotscalecheck(img_stack_deconvolved,num_images,whichframes);
...
for k = 1:num_images;
plane = uint16(img_stack_deconvolved(:,:,k)); %gotta put it back in uint16
if k==1
    imwrite(plane,[save_name '-as4scaled.tif'])
else
    imwrite(plane,[save_name '-as4scaled.tif'],'WriteMode','append') %appending each frame to the first one (multi-image tiff).
end
