...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = at_image_blur();
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = at_image_blur(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
...
for i=1:length(input_finfo),
	...
	if i==1, try, mkdir([getpathname(atd) filesep 'images' filesep output_itemname]); end; end; % Makes new folder if needs too
