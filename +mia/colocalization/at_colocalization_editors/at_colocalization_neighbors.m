...
% 
%  OUT = mia.colocalization.at_colocalization_editors.at_colocalization_neighbors(ATD, INPUT_ITEMNAME, OUTPUT_ITEMNAME, PARAMETERS)
%
...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = mia.colocalization.at_colocalization_editors.at_colocalization_neighbors;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = mia.colocalization.at_colocalization_editors.at_colocalization_neighbors(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
		case 'choose_inputdlg',
			out_p = mia.colocalization.at_colocalization_editors.at_colocalization_neighbors;
...
h(end+1) = struct('parent',input_itemname,'operation','mia.colocalization.at_colocalization_editors.at_colocalization_neighbors','parameters',parameters,...
