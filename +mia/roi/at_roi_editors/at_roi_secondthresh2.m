...
% out = mia.roi.roi_editors.at_roi_secondthresh(ATD,INPUT_ITEMANME,OUTPUT_ITEMNAME,PARAMETERS) 
...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = mia.roi.roi_editors.at_roi_secondthresh2;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = mia.roi.roi_editors.at_roi_secondthresh2(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
		case 'choose_inputdlg',
			out_p = mia.roi.roi_editors.at_roi_secondthresh2;
			...
			if isempty(parameters),
				...
				out = mia.roi.roi_editors.at_roi_secondthresh2(atd,input_itemname,output_itemname,parameters);
			end
	end; % switch
	return;
end;
...
[intensity_thresh,max_neg_slopes,cutoff,highest_pixel] = mia.roi.roi_functions.at_roi_secthreshslopes(atd,ROIname,parameters);
