...
% 
%  OUT = mia.roi.roi_editors.at_roi_volumefilter(ATD, INPUT_ITEMNAME, OUTPUT_ITEMNAME, PARAMETERS)
%
...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = mia.roi.roi_editors.at_roi_clusterfilter;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = mia.roi.roi_editors.at_roi_clusterfilter(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
		case 'choose_inputdlg',
			out_p = mia.roi.roi_editors.at_roi_clusterfilter;
			default_parameters.indexes_to_include = 1;
			parameters = dlg2struct('Choose parameters',out_p{1},out_p{2},default_parameters);
			if isempty(parameters),
				out = [];
			else,
				out = mia.roi.roi_editors.at_roi_clusterfilter(atd,input_itemname,output_itemname,parameters);
			end;
		case 'choose_graphical',
			out = [];
			roi_pfile = getroiparametersfilename(atd,input_itemname);
			ROIp = load(roi_pfile,'-mat');
			o = mia.roi.roi_functions.at_roi_parameters2struct(ROIp.ROIparameters);
...
h(end+1) = struct('parent',input_itemname,'operation','mia.roi.roi_editors.at_roi_clusterfilter','parameters',parameters,...
	'description',['Filtered all but ' int2str(numel(parameters.indexes_to_include)) ' of ROIS ' input_itemname '.']);

mia.roi.roi_functions.at_roi_savesubset(atd,input_itemname, parameters.indexes_to_include, output_itemname, h);
