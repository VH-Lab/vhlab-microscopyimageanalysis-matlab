function out = gui2(name, varargin)
% GUI2 - 2nd generation GUI for Array Tomography analysis
%
%   mia.GUI.gui2 
%
...
if ~command_extract_success,
	error(['Command must include mia.GUI.gui2 name']);
end;
...
elseif ~strcmp(lower(command),'get_vars') & ~strcmp(lower(command),'get_handles'), % let the routine below handle it
        ud = mia.GUI.gui2(name,'command',[name 'Get_Vars']);
end;
...
switch lower(command),
	case 'init',
		...
		uidefs = basicuitools_defs('callbackstr',['callbacknametag(''mia.GUI.gui2'',''' name ''');']);
		...
		mia.GUI.itemeditlist_gui('ROIg','itemtype','ROIs','itemtype_singular','ROI','itemtype_plural','ROIs',...
				'UpperRightPoint',[450 300],'LowerLeftPoint',[5 5],'new_functions',mia.roi.makers_list,'new_items','IMl',...
				'edit_functions',mia.roi.editors_list,'edit_items','ROIg','visiblecbstring','Draw lines',...
				'drawaction','mia.GUI.gui2','drawaction_userinputs',{name,'command',[name 'ATGUI_DrawROIs']},...
				'extracbstring','Overlay','useextracb',1);
		mia.GUI.itemeditlist_gui('IMl','UpperRightPoint',[450 605],'LowerLeftPoint',[5 305],'useedit',0,'usevisible',0,'viewselectiononly',1,...
				'drawaction','mia.GUI.gui2','drawaction_userinputs',{name,'command',[name 'ATGUI_DrawImage']},...
				'new_functions',mia.image.process_list);
		mia.GUI.itemeditlist_gui('COLg','UpperRightPoint',[450+455 300],'LowerLeftPoint',[455 5],...
				'itemtype','CLAs','itemtype_singular','CLA','itemtype_plural','Colocalization Analyses','visiblecbstring','Draw lines',...
				'drawaction','mia.GUI.gui2','drawaction_userinputs',{name,'command',[name 'ATGUI_DrawColocalizations']},...
				'new_functions',mia.colocalization.makers_list,'new_items','ROIg',...
				'edit_functions',mia.colocalization.editors_list,'edit_items','COLg', ...
				'extracbstring','Overlay','useextracb',1);

		image_viewer_gui('IMv','LowerLeftPoint',[455 305],'UpperRightPoint',[455+450 685-5],'showhistogram',0,...
				'drawcompletionfunc',['mia.GUI.gui2(''' name ''', ''command'', [''' name ''' ''ATGUI_ImageMoved'']);']);

		ch = get(fig,'children');
		set(ch,'units','normalized');

		if ~isempty(ud.pathname), 
			mia.GUI.gui2(name,'command',[name 'ATGUI_PathUpdateButton'],'fig',fig);
		end;
	...
	case lower('ATGUI_ImageMoved'),
		mia.GUI.itemeditlist_gui('ROIg','command',['ROIg' 'drawaction'],'fig',fig);
	...
	case lower('ATGUI_DrawROIs'), % NEEDS INPUT ARGUMENT theinput.itemstruct_parameters
		...
		mia.GUI.gui2(name,'command',[name 'ATGUI_DrawROIOverlay'],'fig',fig,'theinput',theinput);
		mia.GUI.gui2(name,'command',[name 'ATGUI_DrawROILines'],'fig',fig,'theinput',theinput);
	...
	case lower('ATGUI_DrawColocalizations'), % NEEDS INPUT ARGUMENT theinput.itemstruct_parameters
		...
		mia.GUI.gui2(name,'command',[name 'ATGUI_DrawCLAOverlay'],'fig',fig,'theinput',theinput);
		mia.GUI.gui2(name,'command',[name 'ATGUI_DrawCLALines'],'fig',fig,'theinput',theinput);
	...
	otherwise,
		disp(['Unknown command to mia.GUI.gui2: ' command ', name = ' name ]);
end;
