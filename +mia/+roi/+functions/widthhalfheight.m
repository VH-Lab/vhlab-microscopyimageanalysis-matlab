%% Local Background
function [local_bg,whh,highest_pixel]= widthhalfheight(mdir,filename,varargin)
% [local_bg,highest_pixel] = MIA.ROI.FUNCTIONS.LOCBACGR(MDIR,FILENAME,VARARGIN) 
...
[dummy,im_fname] = mia.roi.functions.underlying_image(mdir,itemname);
...
% Note that I have to take extreme precautions for edge cases. Such is the
% nature of images, I suppose.
if exist('whhnorth'), clear whhnorth; end
if exist('whheast'), clear whheast; end
if exist('whhwest'), clear whhwest; end
if exist('whhsouth'), clear whhsouth; end
if exist('whhvert'), clear whhvert; end
if exist('whhhorz'), clear whhhorz; end

try
    whhnorth = find(lines.northline <= (highest_pixel(punctum) - (highest_pixel(punctum) - local_bg(punctum))/2),1);
end
try
    whhsouth = find(lines.southline <= (highest_pixel(punctum) - (highest_pixel(punctum) - local_bg(punctum))/2),1);
end
try
    whheast = find(lines.eastline <= (highest_pixel(punctum) - (highest_pixel(punctum) - local_bg(punctum))/2),1);
end
try
    whhwest = find(lines.westline <= (highest_pixel(punctum) - (highest_pixel(punctum) - local_bg(punctum))/2),1);
end

if exist('whhnorth') & exist('whhsouth'),
    if ~isempty(whhnorth) & ~isempty(whhsouth),
    whhvert = whhnorth + whhsouth + 1;
end, end
if exist('whheast') & exist('whhwest'),
    if ~isempty(whheast) & ~isempty(whhwest),
    whhhorz = whhwest + whheast + 1;
end, end

if exist('whhhorz') & exist ('whhvert'), % then use both vert and horz
    whh(punctum) = (whhvert + whhhorz)/2;
elseif exist('whhhorz'), % if only horizontal or vertical exists, just use one
    whh(punctum) = whhhorz;
elseif exist('whhvert'), % ibid
    whh(punctum) = whhvert; 
    
elseif exist('whhnorth') & isempty(whhnorth) == 0,% if only one line exists, just use that line (double it though)
    whh(punctum) = whhnorth*2 + 1;
elseif exist('whhsouth') & isempty(whhsouth) == 0, % it'll stop the first one of these that exists, though
    whh(punctum) = whhsouth*2 + 1;
elseif exist('whheast') & isempty(whheast) == 0,
    whh(punctum) = whheast*2 + 1;
elseif exist('whhwest') & isempty(whhwest) == 0,
    whh(punctum) = whhwest*2 + 1;
end

end % end of calculations for each punctum
close(h)
