function out = at_roi_zspanfilter(atd, input_itemname, output_itemname, parameters)
% AT_ROI_ZSPANFILTER - Filter ROIs by Z span
% 
%  OUT = MIA.ROI.EDITORS.AT_ROI_ZSPANFILTER(ATD, INPUT_ITEMNAME, OUTPUT_ITEMNAME, PARAMETERS)
%
...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = mia.roi.editors.at_roi_zspanfilter;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = mia.roi.editors.at_roi_zspanfilter(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
		case 'choose_inputdlg',
			out_p = mia.roi.editors.at_roi_zspanfilter;
...
h(end+1) = struct('parent',input_itemname,'operation','mia.roi.editors.at_roi_zspanfilter','parameters',parameters,...
