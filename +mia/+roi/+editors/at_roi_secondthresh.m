...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out_choice = mia.roi.editors.at_roi_secondthresh;
			choices = cat(2,out_choice{3},'Cancel');
			buttonname = questdlg('By which method should we choose parameters?',...
				'Which method?', choices{:},'Cancel');
			if ~strcmp(buttonname,'Cancel'),
				out = mia.roi.editors.at_roi_secondthresh(atd,input_itemname,output_itemname,buttonname);
			else,
				out = [];
			end;
		case 'choose_inputdlg',
			out_p = mia.roi.editors.at_roi_secondthresh;
			...
			if isempty(parameters),
				...
				out = mia.roi.editors.at_roi_secondthresh(atd,input_itemname,output_itemname,parameters);
			end
	end; % switch
	return;
end;
