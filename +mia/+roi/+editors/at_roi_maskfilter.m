...
if ischar(parameters),
	switch lower(parameters),
		case 'choose',
			out = at_roi_maskfilter(atd,input_itemname,output_itemname,'choose_inputdlg');
		case 'choose_inputdlg',
			out_p = at_roi_maskfilter;
			default_parameters.mask_file = '';
			parameters = dlg2struct('Choose parameters',out_p{1},out_p{2},default_parameters);
			if isempty(parameters),
				out = [];
			else,
				out = at_roi_maskfilter(atd,input_itemname,output_itemname,parameters);
			end;
	end;
	return;
end;
...
h(end+1) = struct('parent',input_itemname,'operation','at_roi_maskfilter','parameters',parameters,...
